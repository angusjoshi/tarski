...
\begin{proof}
  ...
  $O(\log^{d-1}N)$ queries to solve the $d-1$ dimensional case. The $d$ dimensional algorithm
  will require at most $\log N$ recursive calls to the $d-1$ dimensional algorithm, so the algorithm
  terminates using at most $O(\log N) \cdot O(\log^{d-1} N) = O(\log^d N)$ queries to $f$.
\end{proof}
\begin{theorem}
...
\begin{definition}[Arrival Graph]
  ...
  vertices $s, t$, and a pair of maps 
  $s_0, s_1 : V \cup \{s\} \to V \cup \{s, t\}$. 
\end{definition}
\begin{definition}[Arrival Walk]
  Let $(V, s, t, s_0, s_1)$ be an arrival graph. The \emph{arrival walk}
  on this graph is a sequence of vertices $(v_i)_{i \in \znn} \in V \cup \{s, t\}$
...
\begin{definition}[$\textsc{Arrival}$]
  ...
  decide whether or not the arrival graph ever reaches $t$.
\end{definition}
\begin{figure}[h]
...
just simulate the walk, the efficiency of which depends only on the length of
the walk. As it turns out, there are instances of the arrival
problem where the length of the walk grows like $\Theta(2^{|V|})$ as shown
in \cref{expLongArrival}.
\begin{figure}
...
This idea was used in \citep{gärtner2021subexponential} in a slightly different form
to give a sub-exponential time algorithm for solving the $\textsc{Arrival}$ problem. To
show this reduction I'll need some more definitions.
\begin{definition}[Switching Flow]
  Let $(V, s, t, s_0, s_1)$ be an arrival graph. A \emph{switching flow} is a pair of maps $s$.
\end{definition}
